...
val p07 = suspend {
    ...
    val tree = BagTree()
    input.lines().forEach {
        val elements = it.split("contain")
        val bagColor = elements[0].substringBefore(" bags")
        val contents = Regex("""(\d+) (.+?) bag""").findAll(elements[1]).associate {
            val (count, color) = it.destructured
            color to count.toInt()
        }
        tree.addBag(bagColor, contents)
    }

    tree["shiny gold"].containers().count().print { "Part 1: $it" }
    tree["shiny gold"].countContents().print { "Part 2: $it" }

}

data class Bag(val color: String, var contents: Map<Bag, Int> = mapOf()) {
    val containedIn: MutableSet<Bag> = mutableSetOf()

    fun containers(): Set<Bag> = containedIn + containedIn.flatMap { it.containers() }

    fun countContents(): Int = contents.entries.sumBy { (bag, count) -> count * (1 + bag.countContents()) }
}
...
class BagTree {
    ...
    fun addBag(color: String, contents: Map<String, Int>) {
        val bag = this[color]
        bag.contents = contents.mapKeys { this[it.key] }.onEach { c -> c.key.containedIn += bag }
    }

    operator fun get(color: String) = bags.getOrPut(color) { Bag(color) }
...
val input = """plaid fuchsia bags contain 5 light violet bags, 1 light yellow bag.
