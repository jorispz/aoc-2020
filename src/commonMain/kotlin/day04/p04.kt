...
val p04 = suspend {
    val passports: List<Map<String, String>> = input.split("\n\n").map { line ->
        line.replace("\n", " ").split(" ").associate { pair -> pair.split(":").let { Pair(it[0], it[1]) } }
    }
    ...
    passports
        .count { validators.keys.without("cid").all { key -> it.containsKey(key) } }
        .print { "Part 1: $it valid passwords" }

    passports
        .count { validators.entries.all { (field, validator) -> validator(it[field]) } }
        .print { "Part 2: $it valid passwords" }


}

val test = """ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
