...
// 560
// 303
class PasswordLine(range: IntRange, ch: Char, password: String) {
    val isValid1 = password.count(ch) in range
    val isValid2 = password.hasCharAt(range.first - 1, ch) xor password.hasCharAt(range.last - 1, ch)
}
...
val p02 = suspend {
    ...
    passwordList.count { it.isValid1 }.print { "Part 1: $it valid passwords" }
    passwordList.count { it.isValid2 }.print { "Part 2: $it valid passwords" }

}

fun String.count(c: Char) = this.count { it == c }
fun String.hasCharAt(pos: Int, c: Char) = this[pos] == c


val test = """1-3 a: abcde
1-3 b: cdefg
2-9 c: ccccccccc"""

val input = """4-5 t: ftttttrvts
7-8 k: kkkkkkkf
4-6 k: gqjkkk
1-2 t: rttb
7-10 z: gzjtmtcrzv
